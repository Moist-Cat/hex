...
class HexGameCLI:
    ...
    def get_human_move(self):
        while True:
            try:
                ...
                print(
                    f"Invalid input! Enter two numbers 0-{self.board_size-1} separated by space."
                )
    ...
    def show_stats(self):
        ...
        if self.ai_metrics["move_times"]:
            avg = sum(self.ai_metrics["move_times"]) / len(
                self.ai_metrics["move_times"]
            )
    ...
    def play_game(self):
        ...
        while True:
            self.clear_screen()
            print(
                f"Hex Game (Size: {self.board_size}) - You are {'X (Player 1)' if self.human_player == 1 else 'O (Player 2)'}"
            )
    ...
    def settings_menu(self):
        ...
        print(
            f"2. Player Side (Current: {'Player 1 (X)' if self.human_player == 1 else 'Player 2 (O)'})"
        )
        print("3. Start Game")
